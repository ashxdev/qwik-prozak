...
import { $, useStore, component$, useVisibleTask$ } from "@builder.io/qwik"

export const HeaderCookie = component$(() => {
  const store = useStore({ isShowCookiePolice: false })

  // Only runs in the client
  useVisibleTask$(() => {
    store.isShowCookiePolice = document.cookie.indexOf("accepted_cookies=") < 0
  })
  ...
  return (
    <div>
      <div
        class={clsx([
          { "d-none": !store.isShowCookiePolice },
          { show: store.isShowCookiePolice },
          "alert alert-warning py-2 m-0 bg-primary border-0 rounded-0 alert-dismissible fade text-center overflow-hidden"
        ])}
        role="alert"
      >
        ...
        <div class="position-relative">
          <p class="text-white m-0">
            Я дозволяю PROZAK.INFO використовувати файли{" "}
            <a class="badge badge-primary" href="/cookie-policy">
              cookie
            </a>
            <a
              href="#"
        ...
        <button
          ...
          onClick$={() =>
            (store.isShowCookiePolice = !store.isShowCookiePolice)
          }
        ></button>
      </div>
    </div>
